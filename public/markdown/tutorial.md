...
Claude Code 使用基于配置的权限管理系统，通过三个列表控制工具的执行权限：

**三层权限控制：**

1. **Allow（允许列表）** - 自动执行，无需确认
   - 适用于：安全的只读操作
   - 示例：`Read`、`Glob`、`Grep`
   - 这些工具会立即执行，不会弹出确认对话框
...
1. **项目级配置** - `.claude/settings.json`（当前项目）
2. **用户级配置** - `~/.claude/settings.json`（全局）
3. **默认配置** - Claude Code 内置默认值
